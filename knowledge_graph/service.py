"""知识图谱服务层：封装所有 Neo4j 查询，供 API 路由、Agent 工具、业务服务层统一调用。"""
...
async def fetch_chains() -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(f"读取 Neo4j 产业链列表失败: {e}")
        return []
...
async def graph_chains_with_fallback() -> list[dict[str, Any]]:
    """获取产业链列表，Neo4j 不可用时返回空列表。"""
    return await fetch_chains()


async def fetch_chain_topology(chain_name: str) -> dict[str, Any] | None:
    """查询指定产业链的完整拓扑结构（环节、公司、产品、技术）。"""
    query = """
    ...
    try:
        ...
        logger.warning(f"读取 Neo4j 产业链拓扑失败: chain={chain_name}, error={e}")
        return None
    if not rows:
        return None
...
async def chain_topology_with_fallback(chain_name: str) -> dict[str, Any] | None:
    """获取产业链拓扑，Neo4j 不可用时返回 None。"""
    return await fetch_chain_topology(chain_name)
...
async def search_companies(keyword: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(f"Neo4j 公司搜索失败: {e}")
        return []
